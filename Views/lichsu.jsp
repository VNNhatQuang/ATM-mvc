<%@page import="bean.chitiettaikhoanbean"%>
			...
			<div class="desc">
				<div>
					<table class="table table-hover">
						...
						<%
							ArrayList<chitiettaikhoanbean> lsgd = (ArrayList<chitiettaikhoanbean>) session.getAttribute("lsgd");
							NumberFormat en = NumberFormat.getInstance(new Locale("en", "EN"));
							if(lsgd!=null) {
								for(chitiettaikhoanbean cttk : lsgd) {
						%>
									<tr>
										<td><%=cttk.getSoTaiKhoan() %></td>
										<td><%=en.format(cttk.getSoTienRutRa()) %> VND</td>
										<td><%=cttk.getGhichu() %></td>
										<td><%=cttk.getNgayRutTien() %></td>
									</tr>
								<%} %>
						<%} %>
